"""Run random nearby left-arm reach targets and summarize robustness."""
...
print("\n".join(lines))
(OUTPUT_DIR / "reach_random_left_summary.txt").write_text(
    "\n".join(lines), encoding="utf-8"
)
np.savez_compressed(
    OUTPUT_DIR / "reach_random_left_summary.npz",
    rows=np.array(rows, dtype=object),
    summary=np.array(summary, dtype=object),
    success_rate=np.array(summary["success_rate"]),
)
print("\nSaved to outputs/reach_random_left_summary.txt")
print("Saved to outputs/reach_random_left_summary.npz")
